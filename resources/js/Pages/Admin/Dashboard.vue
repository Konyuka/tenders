<template>
    <div>
        <!-- This example requires Tailwind CSS v2.0+ -->
        <div class="relative bg-gray-50 overflow-hidden">
            <div
                class="hidden sm:block sm:absolute sm:inset-y-0 sm:h-full sm:w-full"
                aria-hidden="true"
            >
                <div class="relative h-full max-w-7xl mx-auto">
                    <svg
                        class="absolute right-full transform translate-y-1/4 translate-x-1/4 lg:translate-x-1/2"
                        width="404"
                        height="784"
                        fill="none"
                        viewBox="0 0 404 784"
                    >
                        <defs>
                            <pattern
                                id="f210dbf6-a58d-4871-961e-36d5016a0f49"
                                x="0"
                                y="0"
                                width="20"
                                height="20"
                                patternUnits="userSpaceOnUse"
                            >
                                <rect
                                    x="0"
                                    y="0"
                                    width="4"
                                    height="4"
                                    class="text-gray-200"
                                    fill="currentColor"
                                />
                            </pattern>
                        </defs>
                        <rect
                            width="404"
                            height="784"
                            fill="url(#f210dbf6-a58d-4871-961e-36d5016a0f49)"
                        />
                    </svg>
                    <svg
                        class="absolute left-full transform -translate-y-3/4 -translate-x-1/4 md:-translate-y-1/2 lg:-translate-x-1/2"
                        width="404"
                        height="784"
                        fill="none"
                        viewBox="0 0 404 784"
                    >
                        <defs>
                            <pattern
                                id="5d0dd344-b041-4d26-bec4-8d33ea57ec9b"
                                x="0"
                                y="0"
                                width="20"
                                height="20"
                                patternUnits="userSpaceOnUse"
                            >
                                <rect
                                    x="0"
                                    y="0"
                                    width="4"
                                    height="4"
                                    class="text-gray-200"
                                    fill="currentColor"
                                />
                            </pattern>
                        </defs>
                        <rect
                            width="404"
                            height="784"
                            fill="url(#5d0dd344-b041-4d26-bec4-8d33ea57ec9b)"
                        />
                    </svg>
                </div>
            </div>

            <div class="relative pt-6 pb-16 sm:pb-24">
                <main class="mt-16 mx-auto max-w-7xl px-4 sm:mt-24">
                    <div class="text-center">
                        <h1
                            class="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl"
                        >
                            <span class="block xl:inline">Bidders</span>
                            <span class="block text-indigo-600 xl:inline"
                                >Portal</span
                            >
                        </h1>
                        <p
                            v-if="!this.$parent.admin"
                            class="text-2xl tracking-tight font-extrabold text-gray-900 sm:text-2xl md:text-2xl"
                        >
                            <span class="block text-indigo-600 xl:inline"
                                >Free</span
                            >
                            <span class="block xl:inline">Membership</span>
                        </p>
                        <p
                            class="mt-3 max-w-md mx-auto text-base text-gray-500 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl"
                        >
                            Welcome {{ this.$parent.user.name }}
                            <br />
                            You are successfully logged in!
                        </p>
                        <!-- Tender Refreshed Successfully -->
                        <div
                            v-if="refreshed"
                            class="grid grid-cols-3 gap-6 mt-10"
                        >
                            <div></div>
                            <div class="rounded-md bg-green-50 p-4">
                                <div class="flex">
                                    <div class="flex-shrink-0">
                                        <svg
                                            class="h-5 w-5 text-green-400"
                                            xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 20 20"
                                            fill="currentColor"
                                            aria-hidden="true"
                                        >
                                            <path
                                                fill-rule="evenodd"
                                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                                clip-rule="evenodd"
                                            />
                                        </svg>
                                    </div>
                                    <div class="ml-3">
                                        <p
                                            class="text-lg font-extrabold text-green-800"
                                        >
                                            Tenders Refreshed Successfully
                                        </p>
                                    </div>
                                    <div class="ml-auto pl-3">
                                        <div class="-mx-1.5 -my-1.5">
                                            <a
                                                :href="route('dashboard')"
                                                type="button"
                                                class="inline-flex bg-green-50 rounded-md p-1.5 text-green-500 hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-green-50 focus:ring-green-600"
                                            >
                                                <span class="sr-only"
                                                    >Dismiss</span
                                                >
                                                <!-- Heroicon name: solid/x -->
                                                <svg
                                                    class="h-5 w-5"
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 20 20"
                                                    fill="currentColor"
                                                    aria-hidden="true"
                                                >
                                                    <path
                                                        fill-rule="evenodd"
                                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                                                        clip-rule="evenodd"
                                                    />
                                                </svg>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div></div>
                        </div>
                        <div
                            class="mt-5 max-w-md mx-auto sm:flex sm:justify-center md:mt-8"
                        >
                            <div class="rounded-md shadow">
                                <button
                                    v-if="this.$parent.admin"
                                    @click="refreshTenders"
                                    class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10"
                                >
                                    <i class="fas fa-rotate mr-2 fa-2x"></i>
                                    Refresh Tenders
                                </button>
                            </div>
                        </div>
                    </div>
                </main>
            </div>
        </div>

        <Spinner v-if="refreshLoader" />
    </div>
</template>

<script>
import Layout from "../../Layouts/DashboardLayout";
import Spinner from "../Components/Spinner";

export default {
    name: "AdminDash",
    layout: Layout,
    components: {
        Spinner
    },
    props: {
        refreshed: Boolean
    },
    data() {
        return {
            refreshLoader: false
        };
    },
    watch: {},
    computed: {},
    methods: {
        refreshTenders() {
            this.refreshLoader = true;
            this.$inertia
                .get("/refresh")
                .then(result => {
                    this.refreshLoader = false;
                })
                .catch(err => {});
        }
    }
};
</script>

<style scoped></style>
